...
Current values of continuous outputs in process systems can be described using a properly selected qualitative range space. To describe a value of a level sensor of a tank, the following range space can be used:

$\{\mathbf{e-},0,L,N,\mathbf{e+}\}$

Here $0$ means an empty tank, $L$ and $N$ means low and normal fluid level respectively, while $\mathbf{e-}$ and $\mathbf{e+}$ refer to unmeasurably low and high fluid levels (this might refer to a failure in the level sensor itself). This range space will be used to describe system outputs during operation.
...
\ref{sec:deviations}
Nominal and characteristic traces can be compared by comparing their correspoinding event fragments. Deviations are formed from a deviation guideword and the nominal event from which the corresponding characteristic trace event is deviationg from. Two main deviation types can be distinguished.
...
\item Temporal deviations. The input-output combination of the event did not happen exactly at the time instant as in the nominal event. 
	...
	\end{itemize}
\item Qualitative value deviations. The output of the event is different from the nominal event. The relational operators are described in \cite{QUALCAL} to compare different qualitative values.
	\begin{itemize}
...
As described in \ref{sec:prochazid}, the diagnosis is based on the fact that a row in the $\mathbf{P-HAZID}$
...
\State $\mathbf{t_{FDP}}$=$\{ \exists (\mathbf{t},d_1) \in \mathbf{DEV}, (\mathbf{t-1},d_2) \in \mathbf{DEV} \}$
...
After executing the algorithm on the nominal and faulty traces and the $\mathbf{P-HAZID}$ table, the following identified root causes ($\mathbf{IRC}$) and non-identified root causes ($\mathbf{INC}$) were obtained using the algorithm with the "Normal operational procedure" during the presence of the single failures (refer to subsection \ref{ref:procsys}).
...
\item Fault VA1-CLOSEFAILURE: Trace was identical, fault could not be detected.

\item Fault VA2-CLOSEFAILURE: Trace was identical, fault could not be detected.
...
\item When the faulty trace is identical to the nominal or too few deviations can be found, then faults cannot be diagnosed (in the case of VA2-CONGESTION, VA1-CLOSEFAILURE or VA2-CLOSEFAILURE).
\item Obviously, when faults are present in component functionality which is not used (in the case of VA1-CLOSEFAILURE or VA2-CLOSEFAILURE) then these faults remain undetectable.
